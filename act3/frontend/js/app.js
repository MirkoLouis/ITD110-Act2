...
function renderStudents(students) {
    ...
    students.forEach(student => {
        ...
        const courseTags = student.courses && student.courses.length > 0
            ? student.courses.map(c => `<span class="course-tag">${escapeHtml(c.courseCode)}</span>`).join('')
            : '<span style="color: #94a3b8;">No courses</span>';
        
        row.innerHTML = `
            <td><span style="font-weight: 600; color: #64748b;">${escapeHtml(student.studentId || '-')}</span></td>
            <td style="font-weight: 500;">${escapeHtml(student.name)}</td>
            <td style="color: #64748b;">${escapeHtml(student.email)}</td>
            <td>${courseTags}</td>
            <td>
