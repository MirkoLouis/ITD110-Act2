...
function renderFaculties(faculties) {
    ...
    faculties.forEach(faculty => {
        ...
        const courseTags = faculty.courses && faculty.courses.length > 0
            ? faculty.courses.map(c => `<span class="course-tag">${escapeHtml(c.courseCode)}</span>`).join('')
            : '<span style="color: #94a3b8;">No courses</span>';
        
        row.innerHTML = `
            <td style="font-weight: 500;">${escapeHtml(faculty.name)}</td>
            <td style="color: #64748b;">${escapeHtml(faculty.address)}</td>
            <td style="color: #64748b;">${escapeHtml(faculty.department)}</td>
            <td>${courseTags}</td>
            <td>
